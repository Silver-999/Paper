...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 23\_Scalable Diffusion Models with Transformers}
    \end{center}
    \vspace{0.8em}
...
c & \R^{d_c} & 时间和类别条件嵌入。\\
        \gamma,\beta,g & \R^d & AdaLN 的尺度、偏置与门。\\
    \bottomrule
    \end{longtable}
...
\section{AdaLN-Zero 条件化}
对条件嵌入 $c$，线性层输出 $(\gamma,\beta,g)$，变换
\begin{equation}
  \operatorname{AdaLN}(h;c)=(1+\gamma(c))\odot\operatorname{LN}(h)+\beta(c),
\end{equation}
残差为
\begin{equation}
  h'=h+g(c)\odot F(\operatorname{AdaLN}(h;c)).
\end{equation}
将门 $g$ 零初始化使每个块初始为恒等映射。类别条件与时间条件通过同一调制入口作用于所有 token，避免额外交叉注意力的二次成本。
...
\end{equation}
在 score 参数化下，这近似把对数密度梯度改为
\begin{equation}
  \nabla\log p(x_t)+ (1+w)\nabla\log p(c\mid x_t),
\end{equation}
增大条件一致性，但 $w$ 过大把采样推向低熵高密度区域，可能损害多样性。
