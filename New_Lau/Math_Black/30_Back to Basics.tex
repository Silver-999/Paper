...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 30\_Back to Basics}
    \end{center}
    \vspace{0.8em}
...
x_\theta,\epsilon_\theta & \R^d & 干净数据与噪声预测。\\
    \bottomrule
    \end{longtable}
...
\section{三种预测参数化的精确换算}
对线性路径 $z_t=t x+(1-t)\epsilon$，速度 $v=x-\epsilon$。联立两式可解得
\begin{equation}
  x=z_t+(1-t)v,\qquad
  \epsilon=z_t-tv,
\end{equation}
...
\end{equation}
因此预测量可以代数互换，但平方误差的权重并不相同：
\begin{align}
  \|v_\theta-v\|_2^2
  &=\frac{1}{(1-t)^2}\|x_\theta-x\|_2^2,\\
  &=\frac{1}{t^2}\|\epsilon_\theta-\epsilon\|_2^2.
\end{align}
若不同时改变时间权重，三种训练目标不等价。

\section{流形假设与输出空间}
设数据位于 $k$ 维流形 $\cM\subset\R^d$，$k\ll d$。干净预测器可写成局部坐标解码
\begin{equation}
  x_\theta(z_t,t)=g_\theta(h_\theta(z_t,t)),
  \qquad h_\theta\in\R^k,quad g_\theta(\R^k)\approx\cM.
\end{equation}
而噪声目标具有满维协方差 $I_d$。这解释了同等容量下直接预测 $x$ 可能更容易，但它不是无条件定理：只有当网络确实能利用低维结构、且时间加权没有在端点处放大误差时，该优势才成立。
