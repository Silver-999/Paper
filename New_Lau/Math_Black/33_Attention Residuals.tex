...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 33\_Attention Residuals}
    \end{center}
    \vspace{0.8em}
...
v_i,k_i & \R^d & 第 $i$ 层历史值与层键。\\
        \alpha_{i\to l} & [0,1] & 第 $l$ 层读取第 $i$ 层的权重。\\
    \bottomrule
    \end{longtable}
...
\section{注意力残差的层间读取}
普通残差递推 $h_l=h_{l-1}+f_l(h_{l-1})$ 展开为
\begin{equation}
  h_l=h_0+\sum_{i=1}^{l}f_i(h_{i-1}),
\end{equation}
所有历史分支系数固定为一。Attention Residuals 改为
\begin{equation}
  h_l=\sum_{i=0}^{l-1}\alpha_{i\to l}v_i,
  \qquad \alpha_{i\to l}=\softmax_i(q_l^{\T}k_i).
\end{equation}
当前层可以选择性读取早期状态，而不是只能访问已经求和、不可逆混合的结果。

\section{梯度通路}
对历史值 $v_i$，直接路径梯度为
\begin{equation}
  \left.\frac{\partial\cL}{\partial v_i}\right|_{\rm direct}
  =\alpha_{i\to l}\frac{\partial\cL}{\partial h_l}.
\end{equation}
另有通过键 $k_i$ 改变权重的间接项。若某层值重要但其键使权重接近零，直接梯度也会被抑制；所以这种机制把残差通路从固定恒等映射变为可学习路由。

\section{缓存复杂度}
保存全部 $L$ 层、$T$ 个 token、宽度 $d$ 的值需要 $O(LTd)$ 内存。若每层查询所有前层，读取量为 $O(L^2Td)$；分组、低秩键或周期性缓存把读取层数限制为 $P$ 后可降为 $O(LPTd)$。
