...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 03\_Flow to the Mode}
    \end{center}
    \vspace{0.8em}
...
\section{FlowMo 的量化潜变量与模式匹配}
编码器先产生 $\hat c=e_\phi(x,c_0)\in\R^m$，逐坐标二值化
\begin{equation}
  c=q(\hat c)=2\,\1\{\hat c\ge0\}-1\in\{-1,1\}^m.
\end{equation}
直通估计把反向导数近似为 $\partial q/\partial\hat c\approx I$。对线性插值
\begin{equation}
  x_t=t z+(1-t)x,\qquad z\sim\cN(0,I),
\end{equation}
真速度是 $z-x$。论文把方向写为 $x-z$ 时，对应的是从 $t=1$ 向 $t=0$ 积分；两种写法仅是时间方向不同，必须与采样器符号一致。流匹配损失为
\begin{equation}
  \cL_{\rm flow}=\E\|d_\theta(x_t,c,t)-(z-x)\|_2^2.
\end{equation}
对任意预测 $\hat v$，由 $x_t=tz+(1-t)x$ 消去 $z$ 可得
\begin{equation}
  \hat x=x_t-t\hat v,
\end{equation}
所以感知损失可以直接施加在估计的干净图像上。

\section{模式寻优后训练为何不同于逐点感知损失}
令 $\Phi_h$ 表示一步 Euler 映射，$n$ 步重建为
\begin{equation}
  \hat x=\Phi_{t_n}\circ\cdots\circ\Phi_{t_1}(z;c).
\end{equation}
后训练目标 $d_{\rm perc}(x,\hat x)$ 的梯度包含整个离散动力系统的链式乘积
\begin{equation}
  \frac{\partial\hat x}{\partial\theta}
  =\sum_{j=1}^n
  \left(\prod_{\ell=j+1}^{n}J_z\Phi_{t_\ell}\right)
  \frac{\partial\Phi_{t_j}}{\partial\theta}.
\end{equation}
它直接选择最终条件分布中靠近原图的模式；逐时刻流损失则拟合条件平均速度，二者优化对象并不相同。

\section{码率与熵正则}
若空间下采样倍率为 $s$，每个潜位置有 $m$ 个二值通道，则未压缩码率为
\begin{equation}
  \mathrm{BPP}=\frac{m}{s^2}.
\end{equation}
对单通道 Bernoulli 概率 $p$，熵
\begin{equation}
  H(p)=-p\log p-(1-p)\log(1-p)
\end{equation}
在 $p=1/2$ 最大；批量边缘熵最大化可防止某些位恒定，而样本内条件熵最小化促使二值决策确定。
