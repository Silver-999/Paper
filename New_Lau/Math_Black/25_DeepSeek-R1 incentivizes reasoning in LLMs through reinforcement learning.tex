...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 25\_DeepSeek-R1 incentivizes reasoning in LLMs through reinforcement learning}
    \end{center}
    \vspace{0.8em}
    ...
    \midrule
    q,o & -- & 提示与完整回答。\\
        \pi_\theta,\pi_{\rm old},\pi_{\rm ref} & -- & 当前、行为与参考策略。\\
        r_j,A_j & \R & 第 $j$ 个回答的奖励与组相对优势。\\
        \rho_{j,t} & \R_{>0} & token 概率比。\\
        \epsilon,\beta & \R_{>0} & 裁剪半径与 KL 权重。\\
    \bottomrule
    \end{longtable}
