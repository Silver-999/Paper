...
LoRI 对任务 $t$ 使用
\begin{equation}
  \Delta W_t=c(B_t\odot M_t)A,
\end{equation}
其中 $A$ 随机初始化后冻结，$M_t\in\{0,1\}^{m\times r}$ 是任务稀疏掩码。若每个元素独立以概率 $s$ 保留，则期望可训练参数为 $smr$，相对 LoRA 的 $r(m+n)$ 进一步下降。

\section{交叉任务干扰的内积}
以 Frobenius 内积衡量两个任务更新的干扰：
\begin{align}
  \langle\Delta W_s,\Delta W_t\rangle_\F
  &=c^2\tr\left((B_s\odot M_s)AA^{\T}(B_t\odot M_t)^{\T}\right).
\end{align}
若随机投影近似满足 $AA^{\T}\approx\sigma_A^2I_r$，则
\begin{equation}
  \langle\Delta W_s,\Delta W_t\rangle_\F
  \approx c^2\sigma_A^2
  \langle B_s\odot M_s,B_t\odot M_t\rangle_\F.
\end{equation}
掩码支持不相交时右侧严格为零；随机稀疏掩码只有“期望重叠小”，并不保证每次合并都正交。

\section{随机投影的尺度}
若 $A_{ij}\sim\cN(0,1/n)$，固定 $x$ 时
\begin{equation}
  \E\|Ax\|_2^2=\frac rn\|x\|_2^2.
\end{equation}
使用 $\sqrt{n/r}A$ 才在期望上保持范数。冻结 $A$ 的作用是固定共享子空间；它减少任务间自由度，也可能在 $r$ 太小时丢失任务需要的方向。
