...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 17\_Mixture-of-Recursions Learning Dynamic Recursive Depths for Adaptive Token-Level Computation}
    \end{center}
    \vspace{0.8em}
    ...
    \midrule
    h_i^{(k)} & \R^d & token $i$ 在第 $k$ 次递归后的状态。\\
        K_i & \{1,\ldots,K_{\max}\} & token 的动态递归深度。\\
        s_i^{(k)} & [0,1] & 继续执行下一次递归的概率。\\
        F_\theta & -- & 多次共享参数的递归块。\\
    \bottomrule
    \end{longtable}
