...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 18\_Native Sparse Attention}
    \end{center}
    \vspace{0.8em}
...
l,n,w & \mathbb N & 压缩块长、选择块数与滑窗宽度。\\
        g_t & \R^3 & 三个稀疏注意力分支的混合门。\\
    \bottomrule
    \end{longtable}
...
\section{三分支稀疏注意力}
NSA 把输出写成压缩、选择和滑窗三支之和
\begin{equation}
  o_t=g_t^{\rm cmp}o_t^{\rm cmp}
  +g_t^{\rm sel}o_t^{\rm sel}
  +g_t^{\rm win}o_t^{\rm win},
\end{equation}
门 $g_t=\softmax(\gamma_t)$ 保证非负且和为一。压缩支把长度为 $l$ 的 KV 块映射为一个代表，数量从 $T$ 降到约 $T/l$；选择支根据压缩注意力分数取 top-$n$ 原始块；滑窗支保留最近上下文。

\section{top-$n$ 选择与训练}
设压缩块概率 $p_j$，块 $j$ 被选择的硬门为 $m_j\in\{0,1\}$。输出
\begin{equation}
  o^{\rm sel}=\softmax(s+\log m)V
\end{equation}
中未选块相当于 $-\infty$。硬 top-$n$ 对分数不可导；训练梯度只穿过被选中的注意力路径，选择器通常依赖共享压缩分支获得间接监督。该机制不等价于对稠密注意力做无偏估计。

\section{复杂度}
若每个查询使用 $n$ 个大小 $l$ 的块和宽度 $w$ 的窗口，注意力边数
\begin{equation}
  O\bigl(T(T/l)+Tnl+Tw\bigr).
\end{equation}
第一项来自查询对压缩块打分；取 $l,n,w$ 为硬件友好常数或缓慢增长量时，主计算显著低于 $T^2$，但索引和不规则访存决定实际速度。
