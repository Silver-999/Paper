...
\section{分隔符稀疏掩码}
对第 $i$ 个查询，仅保留三类键：初始 sink 集合 $S_0$、最近窗口 $W_i$、历史分隔符集合 $P_i$。可见集合
\begin{equation}
  \cN(i)=S_0\cup W_i\cup P_i.
\end{equation}
掩码令 $M_{ij}=0$ 当 $j\in\cN(i)$，否则为 $-\infty$。若窗口宽 $w$、平均每 $b$ 个 token 一个分隔符，则总边数约
\begin{equation}
  \sum_{i=1}^{T}|\cN(i)|
  =O(Tw+T^2/b).
\end{equation}
当分隔符数量随 $T$ 线性增长时，第二项仍是二次；实际加速还需要限制保留的历史分隔符或采用分块缓存。

\section{信息路径长度}
局部窗口使相距 $D$ 的 token 至少需要 $\lceil D/w\rceil$ 层传播；若二者都连接同一分隔符，路径可缩短为两跳。分隔符因而充当可学习的段级摘要，但摘要质量取决于训练时该位置是否被迫聚合段内信息。
