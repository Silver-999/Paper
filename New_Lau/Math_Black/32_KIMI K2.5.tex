...
\begin{document}
    ...
    \midrule
    x,y_j & -- & 提示与第 $j$ 个采样回答。\\
        \pi_\theta,\pi_{\rm old},\pi_{\rm ref} & -- & 当前、旧与参考策略。\\
        r_j,A_j & \R & 奖励与组相对优势。\\
        \lambda_1,\lambda_2 & \R_{\ge0} & 退火的辅助目标权重。\\
    \bottomrule
    \end{longtable}
...
\section{token 级裁剪与辅助项退火}
若回答级奖励 $A_j$ 复制到每个 token，长回答会在 token 平均方式不当时获得更大总权重。使用
\begin{equation}
  \frac1{|o_j|}\sum_t\ell_{j,t}
\end{equation}
先作回答内平均可消除纯长度倍增。若总目标
\begin{equation}
  \cL=\cL_{\rm main}+\lambda_1(s)\cL_{\rm vision}
  +\lambda_2(s)\cL_{\rm swarm},
\end{equation}
并令 $\lambda_1,\lambda_2\to0$，后期不动点由主目标决定；辅助项只改变优化路径和所到达的局部极小值。
