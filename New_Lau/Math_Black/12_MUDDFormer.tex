...
X_i & \R^{T\times d} & 第 $i$ 层隐藏状态。\\
        a_i & \R^{i+1} & 第 $i$ 层的动态跨层权重。\\
    \bottomrule
    \end{longtable}
...
\section{跨层稠密聚合}
设第 $i$ 层可访问 $X_0,\ldots,X_i$，动态权重 $a_i\in\R^{i+1}$ 经 softmax 归一化，
\begin{equation}
  \tilde X_i=\sum_{j=0}^{i}a_{ij}X_j,
  \qquad a_{ij}\ge0,\quad\sum_ja_{ij}=1.
\end{equation}
这使每个 token 的跨层聚合是凸组合。若权重 logits 为 $s_i$，则
\begin{equation}
  \frac{\partial\tilde X_i}{\partial s_{ik}}
  =a_{ik}(X_k-\tilde X_i),
\end{equation}
所以某层状态只有在偏离当前加权均值时才产生较大门控梯度。

\section{与注意力的对应}
若把深度索引看成长度 $i+1$ 的序列，权重可写成
\begin{equation}
  a_i=\softmax(q_iK_i^{\T}),\qquad
  \tilde X_i=a_iV_i.
\end{equation}
普通 token 注意力沿空间维混合；MUDDFormer 的动态稠密连接沿深度维混合。两者矩阵形式相同，但键值的索引语义不同，复杂度分别依赖 $T^2$ 与 $L^2$。
