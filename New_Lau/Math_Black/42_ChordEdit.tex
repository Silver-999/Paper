...
\begin{document}
    ...
    \midrule
    \rho_t & -- & 编辑过程中的概率密度。\\
        u,v,R & \R^d & 控制速度、基础速度与编辑残差。\\
        K_\delta & -- & 时间平滑核。\\
        L,M,h & \R_{>0} & Lipschitz 常数、全导数上界与步长。\\
    \bottomrule
    \end{longtable}
...
\section{控制场分解}
若基础生成 ODE 的速度为 $v(x,t)$，编辑目标引入残差 $R(x,t)$，朴素控制为
\begin{equation}
  u_{\rm nai}=v+R.
\end{equation}
ChordEdit 对时间方向做核平滑
\begin{equation}
  R_\delta(x,t)=(K_\delta*R)(x,t)
  =\int K_\delta(s)R(x,t-s)\,ds,
\end{equation}
并使用 $u_{\rm cho}=v+R_\delta$。若 $\|K_\delta\|_{L^1}=1$，Young 不等式给出
\begin{equation}
  \|R_\delta\|_{L^2}\le\|K_\delta\|_{L^1}\|R\|_{L^2}=\|R\|_{L^2},
\end{equation}
所以平滑不增加 $L^2$ 动能。

\section{方差降低}
设 $R=r+\eta$，噪声零均值且时间白化，核估计噪声方差
\begin{equation}
  \Var[(K_\delta*\eta)(t)]
  =\sigma^2\int K_\delta(s)^2ds.
\end{equation}
对宽度约为 $\delta$ 的归一化核，$\int K_\delta^2=O(1/\delta)$；更宽的核降低方差，却增加对时变真信号 $r$ 的偏差。这是编辑稳定性与语义响应速度之间的偏差--方差权衡。

\section{一步 Euler 误差}
对 Lipschitz 常数 $L$、全导数上界 $M$ 的速度场，一步局部误差
\begin{equation}
  \|x(h)-x_0-hu(x_0,0)\|_2\le\frac{M}{2}h^2.
\end{equation}
多步离散经离散 Gronwall 不等式得到全局 $O(h)$。核平滑同时降低空间 Lipschitz 上界和时间变化上界时，虽不改变阶数，却缩小误差常数，因而更适合单个大步长。
