...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 40\_Improved Baselines with Representation Autoencoders}
    \end{center}
    \vspace{0.8em}
...
z & \R^{N\times d} & 预训练编码器的宽表征潜变量。\\
        P & \R^{d\times d_m} & 潜变量到扩散主干的投影。\\
    \bottomrule
    \end{longtable}
...
\section{高维表征潜变量的尺度}
设编码器输出 $z\in\R^{N\times d}$。逐通道标准化
\begin{equation}
  \tilde z_{ij}=\frac{z_{ij}-\mu_j}{\sqrt{\sigma_j^2+\varepsilon}}
\end{equation}
使每通道方差约为一，因此 $\E\|\tilde z_i\|_2^2\approx d$。若直接把噪声也取 $\cN(0,I_d)$，信号与噪声总能量都随 $d$ 线性增长；优化稳定性取决于每维尺度而不是总范数。

\section{宽表示的投影头}
令扩散主干宽度为 $d_m$，输入投影 $P\in\R^{d\times d_m}$。若 $P_{ij}\sim\cN(0,1/d)$，则固定 $z$ 时
\begin{equation}
  \E\|zP\|_2^2=\frac{d_m}{d}\|z\|_2^2.
\end{equation}
因此初始化尺度必须随输入表征维数调整。宽解码头可提高输出秩上限，但只有训练数据覆盖的子空间会得到可靠约束。

\section{表示对齐引导}
若完整预测为 $\hat x_{\rm full}$，去掉某个表示对齐分支得到 $\hat x_{\rm repa}$，线性引导
\begin{equation}
  \hat x_{\rm guided}=\hat x_{\rm full}
  +w(\hat x_{\rm full}-\hat x_{\rm repa})
\end{equation}
沿“包含该分支所增加的方向”外推。它与分类器自由引导同为差分外推，不能保证对任意 $w$ 保持在训练分布内。
