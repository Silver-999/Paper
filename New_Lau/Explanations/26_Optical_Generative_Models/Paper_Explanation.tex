...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Optical Generative Models}
...
\toprule
题目 & Optical Generative Models\\
课件日期 & 2025-10-16\\
研究方向 & 光计算 / 生成模型\\
一句话概括 & 把生成过程的一部分映射到光学传播与调制器件中,以光的并行传播降低数字生成模型的能耗和延迟。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
把生成过程的一部分映射到光学传播与调制器件中,以光的并行传播降低数字生成模型的能耗和延迟。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item DDPM( 高要求图像生成) 甚至需数百GB 级显存支持
\item DDPM 单张256×256 图像计算能耗可达数百毫焦
\item DDPM 模型生成一张图像需数百毫秒至秒级
\item 三、 生成任务与硬件架构的强绑定限制
\item 传统数字生成模型的三大核心痛点
\item 二、模型规模扩大带来的扩展瓶颈
\end{itemize}
...
\section{核心思想与整体流程}
把生成过程的一部分映射到光学传播与调制器件中,以光的并行传播降低数字生成模型的能耗和延迟。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {输入光场};
\node[procstep,right=of a] (b) {光学调制};
\node[procstep,right=of b] (c) {并行传播};
\node[procstep,right=of c] (d) {探测与生成结果};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item 与DDPM 区别 :训练部分无异,生成推理使用光学系统
\item 静态光学种子生成:随机噪声→相位图案的数字映射
\item 联合训练:数字编码器与光学解码器的协同优化
\item 动态光学解码:自由空间衍射的图像合成过程
\item 关注推理部分,就能把握核心idea
\item 光学生成模型核心创新- 联合训练
\item 看这文章容易只关注训练的部分,实际上推理的部分更重要
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 光学生成模型核心创新 ；关注推理部分，就能把握核心idea ；静态光学种子生成：随机噪声→相位图案的数字映射 ；动态光学解码：自由空间衍射的图像合成过程 ；联合训练：数字编码器与光学解码器的协同优化 ；与DDPM 区别 ：训练部分无异，生成推理使用光学系统
\end{itemize}
\end{keybox}
...
\end{equation}
光学系统利用傅里叶变换和传播传递函数 $H_z$ 在物理空间中并行处理光场。可训练调制器相当于网络参数，探测器读出生成结果。
...
\section{结论}
总体而言，把生成过程的一部分映射到光学传播与调制器件中，以光的并行传播降低数字生成模型的能耗和延迟。。 进一步需要注意：看这文章容易只关注训练的部分,实际上推理的部分更重要;关注“ AI” 之外的内容,比如物理世界基本规律。
