...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Attention Residuals}
...
\toprule
题目 & Attention Residuals\\
课件日期 & 2026-03-15\\
研究方向 & 残差连接 / 注意力聚合\\
一句话概括 & 用分块和注意力加权替代简单逐层相加,使网络能够选择性读取不同深度的历史表示。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
用分块和注意力加权替代简单逐层相加,使网络能够选择性读取不同深度的历史表示。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item h\_l :第l 层的输出(第l 层楼的材料库存)
\item h\_\{l-1\} :第l-1 层的输出(楼下传上来的材料)
\item f\_\{l-1\} :l-1 层的加工函数(楼下这层楼对材料的加工处理)
\item \_\{i l\} α → :注意力权重
\item q\_l = w\_l :第l 层的' 查询向量'
\item k\_i = v\_i :第i 层的' 键向量'
\end{itemize}
...
\section{核心思想与整体流程}
用分块和注意力加权替代简单逐层相加,使网络能够选择性读取不同深度的历史表示。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {历史层状态};
\node[procstep,right=of a] (b) {分块组织};
\node[procstep,right=of b] (c) {注意力残差聚合};
\node[procstep,right=of c] (d) {当前层输出};
...
\item h\_\{l-1\} :第l-1 层的输出(楼下传上来的材料)
\item h\_l :第l 层的输出(第l 层楼的材料库存)
\item AI 没有永恒铁律,理所当然会被不断推翻
\item 美国侧重架构创新,中国重工业落地
\item 理论创新+ 工程落地是未来趋势
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 总结 ；重新思考残差网络 ；从叠加转向深度信息流 ；美国侧重架构创新，中国重工业落地 ； 理论创新+ 工程落地是未来趋势 ；AI 没有永恒铁律，理所当然会被不断推翻
\end{itemize}
\end{keybox}
...
\item q\_l = w\_l :第l 层的' 查询向量'
\item k\_i = v\_i :第i 层的' 键向量'
\item B\_n :第n 个块的层索引集合
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 标准残差连接把上一层表示与当前层变换结果直接相加，使信息可以跨层传播。
\item Attention Residuals 将多个历史层表示组织为候选信息源，再利用注意力权重选择性聚合，而不是对所有历史状态固定求和。
\item 实验阅读时应重点比较深层模型的训练稳定性、长程信息保留能力，以及额外注意力计算带来的开销。
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，用分块和注意力加权替代简单逐层相加，使网络能够选择性读取不同深度的历史表示。。 进一步需要注意：AI 没有永恒铁律,理所当然会被不断推翻;美国侧重架构创新,中国重工业落地。
