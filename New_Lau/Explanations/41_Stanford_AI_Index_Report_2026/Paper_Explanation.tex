...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Stanford AI Index Report 2026}
...
\item 最他原地AI 踏步或向80\%
\item Lau偏科严重算 ()*+,-./0 1
\item Lau偏科严重算 234567
\item Lau偏科严重算 AI 829:;<=>?
\end{itemize}
...
\node[procstep] (a) {数据采集};
\node[procstep,right=of a] (b) {指标分组};
\node[procstep,right=of b] (c) {年度趋势比较};
\node[procstep,right=of c] (d) {产业与政策判断};
...
\item Lau偏科严重算 234567
\item Lau偏科严重算 AI 829:;<=>?
\item Lau偏科严重算 中美模型发布数量对比
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item Lau偏科严重算 最强模型 = 最不透明
\end{itemize}
\end{keybox}
...
\section{实验结果应如何阅读}
\begin{itemize}
\item Lau偏科严重算 Terminal-Bench 和网络安全 Agent 成绩对比
\item Lau偏科严重算 Terminal-Bench 成绩对比
\item Lau偏科严重算 中美专利/ 论文/ 机器人装机量对比
\item Lau偏科严重算 机器人仿真 vs 真实环境成功率对比
\item Lau偏科严重算 网络安全 Agent 成绩对比
\item Lau偏科严重算 AI 数据中心能耗和碳排放
\item Lau偏科严重算 中美模型发布数量对比
\end{itemize}
