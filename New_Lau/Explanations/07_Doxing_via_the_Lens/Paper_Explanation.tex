...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Doxing via the Lens}
...
\toprule
题目 & Doxing via the Lens\\
课件日期 & 2025-05-15\\
研究方向 & 多模态推理 / 图像地理定位 / 隐私安全\\
一句话概括 & 系统研究代理式多模态推理模型如何从背景、文字、建筑和环境线索推断图像位置,并量化由此产生的隐私泄露风险。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
系统研究代理式多模态推理模型如何从背景、文字、建筑和环境线索推断图像位置,并量化由此产生的隐私泄露风险。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item Doxing via the Lens: Revealing Privacy Leakage in Image Geolocation for Agentic Multi-Modal Large Reasoning Model
\item 研究背景与动机 多模态大语言模型(如ChatGPT o3)的发展带来了隐私泄露的新挑战。
\item 现有研究多关注训练数据隐私,而忽视了推理阶段的隐私风险。
\item 本研究聚焦于多模态模型在推理阶段通过图像泄露隐私的问题。
\item 图像地理位置信息可能被恶意利用,如人肉搜索和物理跟踪。
\item 构建隐私敏感数据集,量化隐私泄露风险。
\end{itemize}
...
\section{核心思想与整体流程}
系统研究代理式多模态推理模型如何从背景、文字、建筑和环境线索推断图像位置,并量化由此产生的隐私泄露风险。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {图像输入};
\node[procstep,right=of a] (b) {线索提取};
\node[procstep,right=of b] (c) {代理式检索推理};
\node[procstep,right=of c] (d) {位置与隐私风险};
...
\begin{itemize}
\item Doxing via the Lens: Revealing Privacy Leakage in Image Geolocation for Agentic Multi-Modal Large Reasoning Model
\item 结论与未来方向 ChatGPT o3在图像地理位置推理中存在显著隐私泄露风险。
\item 街道布局和前院设计是关键线索,显著影响地理位置推断。
\item 未来研究应关注隐私保护技术的具体实现方法。
\item 简单视觉遮挡可以显著降低隐私泄露风险。
\item 需要开发更全面的隐私保护技术。
\item 提出方法的优劣势 优势1:首次系统性研究多模态大语言模型在图像地理位置推理中的隐私风险。
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 3 提出方法的优劣势 优势1：首次系统性研究多模态大语言模型在图像地理位置推理中的隐私风险
\item 优势2：构建了包含50张真实世界图像的隐私敏感数据集，用于量化隐私泄露风险
\item 优势3：通过遮挡实验，验证了简单视觉遮挡对降低隐私泄露的有效性
\item 劣势1：研究仅限于ChatGPT o3模型，未涵盖其他多模态模型
\end{itemize}
\end{keybox}
...
\item 提出方法的优劣势 优势1:首次系统性研究多模态大语言模型在图像地理位置推理中的隐私风险。
\item 优势2:构建了包含50张真实世界图像的隐私敏感数据集,用于量化隐私泄露风险。
\item 设计四种不同类型的提示文本,用于查询ChatGPT o3的地理位置预 测。
\item 劣势1:研究仅限于ChatGPT o3模型,未涵盖其他多模态模型。
\item 优势3:通过遮挡实验,验证了简单视觉遮挡对降低隐私泄露的有效性。
\item 研究方法与实验设计 构建包含50张真实世界图像的隐私敏感数据集。
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 6 实验结果与分析 60\\%的预测结果与真实位置偏差在1英里以内，表明街级精度
\item 84\\%的预测结果在5英里以内，表明街区级精度
\item 街道布局和前院设计是关键线索，显著影响地理位置推断
\item 简单视觉遮挡可以显著降低隐私泄露风险
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，系统研究代理式多模态推理模型如何从背景、文字、建筑和环境线索推断图像位置，并量化由此产生的隐私泄露风险。。 进一步需要注意：Doxing via the Lens: Revealing Privacy Leakage in Image Geolocation for Agentic Multi-Modal Large Reasoning Model;结论与未来方向 ChatGPT o3在图像地理位置推理中存在显著隐私泄露风险。。
