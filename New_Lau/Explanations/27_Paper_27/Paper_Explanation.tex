...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily 大模型后门与数据窃取}
...
课件日期 & 2025-10-20\\
研究方向 & 模型安全 / 后门攻击 / 数据提取\\
一句话概括 & 分析攻击者如何通过后门微调把“记忆并泄露下游数据”的行为植入模型,以及常规微调为何难以彻底消除该风险。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
分析攻击者如何通过后门微调把“记忆并泄露下游数据”的行为植入模型,以及常规微调为何难以彻底消除该风险。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item 开源模型微调的三大认知误区 一、微调仅改任务适配性,不泄露私有数据 二、监控模型输出异常,就能发现数据窃取 三、增微调轮次或数据量,能降低泄露风险
\item 后门训练的两种关键方式:SFT 与RL 如何强化数据窃取能力?
\item 实验验证:从模型规模到场景差异,提取性能的量化证据
\item 认知误区:为何“常规微调防护”无法抵御数据窃取?
\item 后门攻击的核心逻辑:从植入到数据提取的完整链条
\item 给模型发 “提取训练数据” 的指令 防御性有限
\end{itemize}
...
\section{核心思想与整体流程}
分析攻击者如何通过后门微调把“记忆并泄露下游数据”的行为植入模型,以及常规微调为何难以彻底消除该风险。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {后门数据构造};
\node[procstep,right=of a] (b) {恶意训练};
\node[procstep,right=of b] (c) {触发查询};
\node[procstep,right=of c] (d) {私有数据提取};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item 监控或者调整微调参数并不能抵御后门数据窃取
\item 论文的价值在于跳出微调本身安全的思维
\item 训练强化、提取触发全流程揭示
\item 后门训练的两种关键方式:SFT 与RL 如何强化数据窃取能力?
\item 实验验证:从模型规模到场景差异,提取性能的量化证据
\item 认知误区:为何“常规微调防护”无法抵御数据窃取?
...
\begin{keybox}{结构解析}
\begin{itemize}
\item 如何应对“隐蔽”威胁 ；防御手段有限，现有方法都存在明显局限性 ；给模型发 “提取训练数据” 的指令 防御性有限 ；“检查模型参数”“少调几轮模型” 作用不大
\end{itemize}
\end{keybox}
...
\item 认知误区:为何“常规微调防护”无法抵御数据窃取?
\item 后门攻击的核心逻辑:从植入到数据提取的完整链条
\item 防御困境与破局方向:如何应对这一隐蔽威胁?
\item Alpaca 数据集里 万条样本 ,只有797 个不一样的开头词
\item 实验验证50 个高频开头词,多数模型提取率超百分之50
\end{itemize}
...
\section{结论}
总体而言，分析攻击者如何通过后门微调把“记忆并泄露下游数据”的行为植入模型，以及常规微调为何难以彻底消除该风险。。 进一步需要注意：开源模型微调的数据泄露风险是“后门植入+ 评估缺失”的结 果,需从体系层面解决;后门训练的两种关键方式:SFT 与RL 如何强化数据窃取能力?。
