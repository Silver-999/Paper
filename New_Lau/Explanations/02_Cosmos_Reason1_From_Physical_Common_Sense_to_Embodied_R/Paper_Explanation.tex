...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Cosmos-Reason1: From Physical Common Sense to}
...
\begin{itemize}
\item 结论 Cosmos-Reason1: From Physical Common Sense To Embodied Reasoning
\item Physical AI Reasoning
\item Why Cosmos-Reason1?
\end{itemize}
...
\node[procstep] (a) {视觉与环境输入};
\node[procstep,right=of a] (b) {物理常识建模};
\node[procstep,right=of b] (c) {具身推理};
\node[procstep,right=of c] (d) {动作决策};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item NVIDIA Cosmos是一个整合前沿生成式世界基础模型 (WFM)、 先进分词器、护栏以及用于加速数据处理和治理的高效工作流的集 成平台
\item 强调智能行为需要通过身体与环境的交互来实现,而不仅仅依 赖“大脑”的运算。
\item 旨在加速物理 AI 系统的开发,如自动驾驶汽车 (AVs) 和机器人。
\item 世界模型生成的内容可以用于训练机器人或LLM
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 什么是Cosmos？
\item NVIDIA Cosmos是一个整合前沿生成式世界基础模型 (WFM)、 先进分词器、护栏以及用于加速数据处理和治理的高效工作流的集 成平台 2.旨在加速物理 AI 系统的开发，如自动驾驶汽车 (AVs) 和机器人
\item 世界模型生成的内容可以用于训练机器人或LLM
\end{itemize}
\end{keybox}
...
\begin{itemize}
\item Why Cosmos-Reason1？
\item 摘要 1.引言 2.Physical AI Reasoning 3.Cosmos-Reason1 4.Data 5.基准 6.实验 7.相关工作 8.结论 3 Cosmos-Reason1: From Physical Common Sense To Embodi……
\end{itemize}
\end{notebox}
