...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Continuous Thought Machines(CTM)}
...
\toprule
题目 & Continuous Thought Machines(CTM)\\
课件日期 & 2026-05-14\\
研究方向 & 时序神经网络 / 连续思维\\
一句话概括 & 把神经元激活的时机和跨时间同步纳入计算,使模型通过内部动态过程逐步形成“思维”,而非只依赖固定层堆叠。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
把神经元激活的时机和跨时间同步纳入计算,使模型通过内部动态过程逐步形成“思维”,而非只依赖固定层堆叠。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item 与Transformer 对比
\item 近年模型很实用,远离了生物机制
\end{itemize}
...
\section{核心思想与整体流程}
把神经元激活的时机和跨时间同步纳入计算,使模型通过内部动态过程逐步形成“思维”,而非只依赖固定层堆叠。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {输入刺激};
\node[procstep,right=of a] (b) {神经元时序动态};
\node[procstep,right=of b] (c) {同步与内部迭代};
\node[procstep,right=of c] (d) {决策输出};
...
\begin{itemize}
\item 与Transformer 对比
\item 未在大模型进行验证,但提供新的研究思路
\item 神经元动态同步,时间维度协调实现推理
\item 自注意力机制,全局关联建模序列依赖
\item 近年模型很实用,远离了生物机制
\item ” ” 思考时间轴可异步、多步骤处理信息
\item CTM 神经元能访问历史记录,自我反思
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 思考与展望 ； 深度学习源于对大脑的模仿 ； 近年模型很实用，远离了生物机制 ； CTM 引入了神经科学洞见 ； CTM 存在的问题： ；神经元同步的计算开销 ；大规模训练的稳定性 ；与现有生态的兼容性
\end{itemize}
\end{keybox}
...
\item 自注意力机制,全局关联建模序列依赖
\item 与Transformer 对比
\item 草图、步骤、验证、结论,具有更好的可解释性
\item Transformer 与CTM 的区别
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 与Transformer 对比 ；Transformer ： ；自注意力机制，全局关联建模序列依赖 ；已在大模型进行验证 ；CTM ；神经元动态同步，时间维度协调实现推理 ；未在大模型进行验证，但提供新的研究思路
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，把神经元激活的时机和跨时间同步纳入计算，使模型通过内部动态过程逐步形成“思维”，而非只依赖固定层堆叠。。 进一步需要注意：草图、步骤、验证、结论,具有更好的可解释性;Transformer 与CTM 的区别。
