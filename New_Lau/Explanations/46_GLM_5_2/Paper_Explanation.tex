...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily GLM 5.2}
...
题目 & GLM 5.2\\
课件日期 & 2026-06-18\\
研究方向 & 长上下文 / 编程模型 / IndexShare\\
一句话概括 & 围绕超长上下文和代码能力,介绍 IndexShare 等索引共享机制如何降低长序列注意力成本,并配合推理工程提升可部署性。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
围绕超长上下文和代码能力,介绍 IndexShare 等索引共享机制如何降低长序列注意力成本,并配合推理工程提升可部署性。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item 能力实测:国产最强Coding 模型
\item 架构核心:IndexShare—— 用1/3 计算量撑起1M 上下文
\item 推理工程:让1M 上下文跑得起、扛得住
\item “ ” 稀疏注意力的隐形瓶颈
\item IndexShare :每4 层共享一次索引
\item 额外惊喜:MTP 也提了20\% !
\end{itemize}
...
\section{核心思想与整体流程}
围绕超长上下文和代码能力,介绍 IndexShare 等索引共享机制如何降低长序列注意力成本,并配合推理工程提升可部署性。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {1M 上下文};
\node[procstep,right=of a] (b) {索引共享};
\node[procstep,right=of b] (c) {高效注意力};
\node[procstep,right=of c] (d) {代码与代理输出};
...
\begin{keybox}{结构解析}
\begin{itemize}
\item 云组会点评 ； GLM5.2 ：做真正可用的长程开源模型 ； 1M 稳定上下文、追平闭源的长程编码能力、MIT 完全开源无限制 ； 当下海外模型高不确定性，GLM 具有重要意义 ； GLM 还在持续进化当中
\end{itemize}
\end{keybox}
...
\section{实验结果应如何阅读}
\begin{itemize}
\item HiSparse “ ” :分层内存,冷数据挪出GPU
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 参考内容 ； ； IndexShare ： ； LayerSplit ： ； HiSparse
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，围绕超长上下文和代码能力，介绍 IndexShare 等索引共享机制如何降低长序列注意力成本，并配合推理工程提升可部署性。。 进一步需要注意：架构核心:IndexShare—— 用1/3 计算量撑起1M 上下文;1M 稳定上下文、追平闭源的长程编码能力、MIT 完全开源无限制。
