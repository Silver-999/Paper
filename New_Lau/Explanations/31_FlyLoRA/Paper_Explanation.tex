...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily FlyLoRA}
...
\begin{itemize}
\item LoRA 与MoE-LoRA 回顾及与FlyLoRA 初步对比
\item FlyLoRA 优势:同步解决干扰与效率问题
\item 果蝇嗅觉工作第三步:选强弃弱,精准激活信号
\item MoE-LoRA :专家模块+ 动态路由器
\item MoE-LoRA 痛点:路由器开销大效率低
\item FlyLoRA 创新:借鉴果蝇嗅觉系统机制
\end{itemize}
...
\node[procstep] (a) {输入特征};
\node[procstep,right=of a] (b) {随机稀疏投影};
\node[procstep,right=of b] (c) {强响应选择};
\node[procstep,right=of c] (d) {低秩适配};
...
\begin{itemize}
\item LoRA 与MoE-LoRA 回顾及与FlyLoRA 初步对比
\item 矩阵 A 维度与初始化:r 行 n 列,元素稀疏且固定
\item FlyLoRA 优势:同步解决干扰与效率问题
\item 矩阵 A 功能一:保持语义距离,保障决策一致
\item 矩阵 A 功能二:实现无参路由,模拟生物机制
\item FlyLoRA 创新:借鉴果蝇嗅觉系统机制
\item 矩阵 A 是 FlyLoRA 无参导航核心
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item LoRA 与MoE-LoRA 回顾及与 初步对比 ；基础LoRA ：低秩矩阵模拟权重更新 ；基础LoRA 痛点：秩选择两难与参数干扰 ；MoE-LoRA ：专家模块+ 动态路由器 ；MoE-LoRA 痛点：路由器开销大效率低 ；传统方法：难平衡干扰与效率损耗
\end{itemize}
\end{keybox}
...
\begin{itemize}
\item LoRA 与MoE-LoRA 回顾及与FlyLoRA 初步对比
\item 果蝇嗅觉系统对FlyLoRA 的启发映射
\item 矩阵 B 拆分:r 列对应 r 个秩级专家,单维度特征学习
\item 秩级专家激活:矩阵 B 为特征学习核心,d 修正关键
\item 激活逻辑:叠 d 后选前 k 专家,稀疏激活降开销
\item 叠加偏置项 d :平衡专家激活概率,避免样本偏置
\item FlyLoRA 优势:同步解决干扰与效率问题
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 秩级专家 ；秩级专家激活：矩阵 B 为特征学习核心，d 修正关键 ；叠加偏置项 d ：平衡专家激活概率，避免样本偏置 ；矩阵 B 拆分：r 列对应 r 个秩级专家，
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，借鉴果蝇嗅觉系统的随机稀疏连接和强响应选择，为不同任务构造高效、低干扰的 LoRA 专家。。 进一步需要注意：LoRA 与MoE-LoRA 回顾及与FlyLoRA 初步对比;透过与经典方法的对比,突出idea 创新点。
