...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily LSNet}
...
课件日期 & 2025-10-07\\
研究方向 & 轻量视觉网络 / 大小核协同\\
一句话概括 & 通过大核感知全局结构、小核动态适配局部位置,在轻量计算预算下兼顾感受野和细节建模。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
通过大核感知全局结构、小核动态适配局部位置,在轻量计算预算下兼顾感受野和细节建模。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item 特征图到点对点卷积居然是用的Reshape
\item 论文在表8 讨论了SE 模块问题,考虑全面
\item 同行质疑SE 模块是否占主要作用
\item 轻量化视觉模型的三大认知误区
\item 点对点小卷积核部分实现难度大
\end{itemize}
...
\section{核心思想与整体流程}
通过大核感知全局结构、小核动态适配局部位置,在轻量计算预算下兼顾感受野和细节建模。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {输入特征};
\node[procstep,right=of a] (b) {大核感知};
\node[procstep,right=of b] (c) {位置相关小核};
\node[procstep,right=of c] (d) {分类/检测};
...
\begin{keybox}{结构解析}
\begin{itemize}
\item 7 LSNet 的整体架构 ；一、输入阶段（Stem ） ；嵌入，投影，下采样 ；二、核心阶段（LS ） ；三个LS Block 堆叠 ；三、最后阶段（MSA ） ；注意力机制
\end{itemize}
\end{keybox}
...
\section{实验结果应如何阅读}
\begin{itemize}
\item 实验结果- 基础分类任务(ImageNet-1K )
\item 实验结果- 下游任务迁移(COCO/ADE20K )
\item 轻量化视觉模型的三大认知误区
\end{itemize}
...
\section{结论}
总体而言，通过大核感知全局结构、小核动态适配局部位置，在轻量计算预算下兼顾感受野和细节建模。。 进一步需要注意：特征图到点对点卷积居然是用的Reshape;论文在表8 讨论了SE 模块问题,考虑全面。
