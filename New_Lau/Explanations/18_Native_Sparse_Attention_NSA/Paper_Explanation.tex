...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Native Sparse Attention(NSA)}
...
\toprule
题目 & Native Sparse Attention(NSA)\\
课件日期 & 2025-08-01\\
研究方向 & 长上下文 / 原生稀疏注意力\\
一句话概括 & 以压缩、选择和滑动窗口三条分支共同建模全局与局部上下文,并从训练到推理保持原生稀疏和硬件友好。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
以压缩、选择和滑动窗口三条分支共同建模全局与局部上下文,并从训练到推理保持原生稀疏和硬件友好。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item NSA 目标:实现高效且可训练的长文本稀疏注意力
\item 长上下文应用(代码生成、多轮对话)需求激增
\item 全注意力在64k 序列上成本过高,难以落地
\item 现有方法缺乏端到端训练支持,性能损失显著
\item 现有稀疏方法存在推理加速不全面的问题
\end{itemize}
...
\section{核心思想与整体流程}
以压缩、选择和滑动窗口三条分支共同建模全局与局部上下文,并从训练到推理保持原生稀疏和硬件友好。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {长序列输入};
\node[procstep,right=of a] (b) {压缩/选择/窗口};
\node[procstep,right=of b] (c) {门控融合};
\node[procstep,right=of c] (d) {长上下文输出};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item 压缩分支:聚合连续token ,捕捉全局粗粒度信息
\item 选择分支:基于压缩结果,保留关键细粒度token
\item NSA 提出动态分层稀疏策略,平衡全局与局部信息
\item 原生可训练特性确保性能不下降,甚至超越全注意力
\item 在通用、长文本、推理任务上均表现优异,鲁棒性强
\item 硬件对齐设计实现了训练和推理全阶段的显著加速
\item 动态分层稀疏策略:压缩+ 选择+ 滑动窗口
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 5 NSA 的算法模块案例详解 ； " 小明今天去学校，学习编程设计，还和同学讨论了作业，明天要交代码" ； 1. 压缩分支 ； 2. 选择分支 ； 3. 滑动窗口分支 ； 4. 门控机制
\end{itemize}
\end{keybox}
...
\section{实验结果应如何阅读}
\begin{itemize}
\item 计算量节省:每query 仅处理约2560 个token (全注意力需64k )
\item 长文本检索:64k“Needle-in-a-Haystack” 准确率100\%
\item 核函数效率:Triton 实现接近FlashAttention 的硬件利用率
\item 训练加速:64k 长度时,前向9.0× 、反向6.0× 于全注意力
\item 长文本任务:LongBench 平均得分0.469 ,超所有基线
\item 解码加速:64k 长度时,内存访问减少11.6× ,速度提升显著
\item 通用任务:MMLU 、GSM8K 等7 项指标超全注意力
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 15 NSA 的实验验证 ； 通用任务：MMLU 、GSM8K 等7 项指标超全注意力 ； 长文本任务：LongBench 平均得分0.469 ，超所有基线 ； 推理任务：AIME 数学推理准确率显著高于全注意力 ； 长文本检索：64k“Needle-in-a-H……
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，以压缩、选择和滑动窗口三条分支共同建模全局与局部上下文，并从训练到推理保持原生稀疏和硬件友好。。 进一步需要注意：NSA 提出动态分层稀疏策略,平衡全局与局部信息;原生可训练特性确保性能不下降,甚至超越全注意力。
