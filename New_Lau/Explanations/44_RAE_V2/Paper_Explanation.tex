...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily RAE V2}
...
\begin{itemize}
\item 背景:VAE 的致命缺陷与第一代RAE 的遗憾
\item 三个核心改进:RAE V2 的底层逻辑
\item 实验验证:用数据说话的全面碾压
\end{itemize}
...
\node[procstep] (a) {语义编码器};
\node[procstep,right=of a] (b) {潜空间校准};
\node[procstep,right=of b] (c) {扩散 Transformer};
\node[procstep,right=of c] (d) {高保真解码};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item MLP 层让扩散模型学到的特征和预训练ViT 保持一致
\item REPA :简单高效的表征对齐正则化方法
\item 改进二:RAE 与REPA 互补
\item 加快训练速度,提升生成效果。
\item EPFID@k:达到无引导gFID k ≤所需的训练epoch 数
\item 有引导生成:依托文本描述等外部指 令约束创作方向生成对应图像。
\item 256×256 无引导FID 达到1.51 ,有引 导FID
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 改进二：RAE 与REPA 互补 ；REPA ：简单高效的表征对齐正则化方法 ；MLP 层让扩散模型学到的特征和预训练ViT 保持一致 ；加快训练速度，提升生成效果
\end{itemize}
\end{keybox}
...
\begin{itemize}
\item EPFID@k:达到无引导gFID k ≤所需的训练epoch 数
\item 训练到满足指定优质画质标准k 时,所需要的迭代轮数。
\item 背景:VAE 的致命缺陷与第一代RAE 的遗憾
\item 三个核心改进:RAE V2 的底层逻辑
\item 实验验证:训练效率:10 倍加速
\item 实验验证:用数据说话的全面碾压
\item 传统收敛速度评判只参考固定轮次
\end{itemize}

\begin{notebox}{结果解读}
\begin{itemize}
\item 重建质量：媲美专有VAE ；图像生成是从0 到1 ，模型根据随机噪声、文本提示，凭空创造 出全新的图像内容 ；图像重建是从1 到1 ，模型将原始图像压缩为隐空间特征，
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，改进第一代表征自编码器的编码、解码和潜空间归一化，使高语义表示更适合作为 DiT 的生成空间。。 进一步需要注意：RAE 让生成模型在视觉理解模型的语义空间里工作;背景:VAE 的致命缺陷与第一代RAE 的遗憾。
