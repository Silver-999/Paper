...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Anthropic 大模型机制可解释性研究}
...
\item “ ” 在模型的中间层接特征分解器
\item 拆分在神经元上叠在一起的特征
\item 人类首次对LLM 内部精准定位与干预
\item LLM 可被别有用心的人利用
\item Templeton, A., Conerly, T., Marcus, J., et al. (2024). *Scaling monosemanticity: Extracting interpretable features from Claude Sonnet*. Transformer Circuits Thread.
\item Lu, C., Gallagher, J., Michala, J., Fish, K., \& Lindsey, J. (2026). *The assistant axis: Situating and stabilizing the character of large language models* [ArXiv Preprint]. arXiv:2601.10387v1 [cs.CL].
\end{itemize}
...
\node[procstep] (a) {模型激活};
\node[procstep,right=of a] (b) {稀疏特征分解};
\node[procstep,right=of b] (c) {因果回路追踪};
\node[procstep,right=of c] (d) {行为解释};
...
\item Templeton, A., Conerly, T., Marcus, J., et al. (2024). *Scaling monosemanticity: Extracting interpretable features from Claude Sonnet*. Transformer Circuits Thread.
\item Lu, C., Gallagher, J., Michala, J., Fish, K., \& Lindsey, J. (2026). *The assistant axis: Situating and stabilizing the character of large language models* [ArXiv Preprint]. arXiv:2601.10387v1 [cs.CL].
\item Ronge, R., Maier, M., \& Eberhardt, F. (2026). *When the coffee feature activates on coffins: An analysis of feature extraction and steering for mechanistic interpretability* [ArXiv Preprint]. arXiv:2601.03047v1 [cs.LG].
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 参考文献 1. Elhage, N., Hume, T., Olsson, C., Schiefer, N., Henighan, T., Kravec,
\end{itemize}
\end{keybox}
...
\begin{notebox}{结果解读}
\begin{itemize}
\item 预测北京房价 ；房间数量、犯罪率、学区质量 ； 房价= 0.5 × 房间数- 0.3 × 犯罪率 ； 房间↑则房价↑ ； 犯罪率↑则房价↓
\end{itemize}
\end{notebox}
