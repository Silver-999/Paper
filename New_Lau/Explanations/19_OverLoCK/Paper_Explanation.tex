...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily OverLoCK}
...
课件日期 & 2025-08-14\\
研究方向 & 卷积网络 / 动态上下文\\
一句话概括 & 模拟“先概览、再聚焦”的视觉过程,以分支结构和动态卷积融合全局上下文与局部细节。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{infobox}{摘要要点}
模拟“先概览、再聚焦”的视觉过程,以分支结构和动态卷积融合全局上下文与局部细节。 本说明按“研究问题—核心机制—基础公式—实验阅读—局限与优化”的顺序重新组织，不保留原始材料的封面、目录和页码对应。
\end{infobox}

\section{研究问题与动机}
这项工作的出发点不是简单地替换某个网络层，而是识别现有范式中最影响\term{质量、效率、可靠性或可部署性}的瓶颈。结合论文所讨论的问题，可以将研究动机概括为以下几点：
\begin{itemize}
\item 和过去的CNN 方法有什么区别?
\item 传统CNN :固定层级传递信息
\item OverLoCK :分支结构模拟人类视觉
\item 动态卷积:融合全局与局部信息
\item 和过去CNN 的方法有什么区别?
\item 视觉机制:从 “盲目拼接” 到 “目标导向”
\end{itemize}
...
\section{核心思想与整体流程}
模拟“先概览、再聚焦”的视觉过程,以分支结构和动态卷积融合全局上下文与局部细节。 从信息流角度看，其基本流程可以整理为下图。
...
\node[procstep] (a) {全局概览};
\node[procstep,right=of a] (b) {上下文建模};
\node[procstep,right=of b] (c) {动态局部卷积};
\node[procstep,right=of c] (d) {视觉预测};
...
\begin{keybox}{结构解析}
\begin{itemize}
\item 3 案例理解核心idea ； 人类视觉：先概览再聚焦 ； 传统CNN ：固定层级传递信息 ； ：分支结构模拟人类视觉 ； 动态卷积：融合全局与局部信息 ； 性能超越现有主流模型
\end{itemize}
\end{keybox}
...
\section{实验结果应如何阅读}
\begin{itemize}
\item 鲁棒性 :从 “局部敏感” 到 “全局抗干扰”
\item 效率 :从 “全面扫描” 到 “聚焦计算”
\item 性能 :准确率与计算量的更优平衡
\item OverLoCK :分支结构模拟人类视觉
\item 和过去的CNN 方法有什么区别?
\item 传统CNN :固定层级传递信息
\item 动态卷积:融合全局与局部信息
...
\begin{itemize}
\item 11 有什么提升？
\item ； 效率 ：从 “全面扫描” 到 “聚焦计算” ； 鲁棒性 ：从 “局部敏感” 到 “全局抗干扰” ； 性能 ：准确率与计算量的更优平衡
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，模拟“先概览、再聚焦”的视觉过程，以分支结构和动态卷积融合全局上下文与局部细节。。 进一步需要注意：和过去的CNN 方法有什么区别?。
