...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Why Language Models Hallucinate}
...
\begin{itemize}
\item 二元评分基准MMLU-Pro 、GPQA , WildBench 会给 IDK 打 3-4 分,但还是比 “有幻觉的回答”(5-6 分)低
\item SWE-bench (软件测试)补丁过测试得 分,否则 分,模 型倾向选择“编代码”,严肃场景高危险性
\item 加个IDK (I don't know) 选项,就能减少幻觉?
\item ChatGPT 、DeepSeek 、Llama 均答错
\item DeepSeek-V3 给出2 、3 、6 、7 等答案
\item 给模型加个‘我不知道’(IDK )选项,就能减少幻觉
\end{itemize}
...
\node[procstep] (a) {知识不确定性};
\node[procstep,right=of a] (b) {生成决策};
\node[procstep,right=of b] (c) {评估奖励};
\node[procstep,right=of c] (d) {回答或拒答};
...
\subsection{方法组成与信息流}
\begin{itemize}
\item “二分类误差→生成错误”催生幻觉
\item 加个IDK (I don't know) 选项,就能减少幻觉?
\item 模型幻觉现象测试:生日,字母计数,论文标题,多次输出错误 答案
\item 模型质量与错误观察:以推理能力为代表的模型质量会影响错误 率
\item 人类反馈融合:将人类对“不确定性表达”的偏好融入训练
\item 模型校准数据引用:预训练目标与模型错误有关联。
\item 动态阈值调整:根据任务风险动态调整置信度阈值
\end{itemize}

\begin{keybox}{结构解析}
\begin{itemize}
\item 幻觉率的量化关键发现 ；模型幻觉现象测试：生日，字母计数，论文标题，多次输出错误 答案 ；模型校准数据引用：预训练目标与模型错误有关联
\item ；基准测试机制分析：二元评分会强化幻觉 ；模型质量与错误观察：以推理能力为代表的模型质量会影响错误 率
\end{itemize}
\end{keybox}
...
\begin{notebox}{结果解读}
\begin{itemize}
\item 评估体系如何“奖励”幻觉 ；二元评分引发幻觉的必然：回答问题，猜比不说强 ；二元评分基准MMLU-Pro 、GPQA ， WildBench 会给 IDK 打 3-4 分，
\end{itemize}
\end{notebox}
...
\section{结论}
总体而言，从预训练中的二分类误差与后训练评估激励解释语言模型为何倾向于猜测，并讨论允许拒答和改造评分机制的价值。。 进一步需要注意：加个IDK (I don't know) 选项,就能减少幻觉?;允许AI“ 说不知道”,给予合理的激励,才能走向“可靠”。
