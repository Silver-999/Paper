...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 1-1810.04805v2-Basis-BERT}
    \end{center}
    \vspace{0.8em}
...
M & 2^{\{1,\ldots,T\}} & MLM 掩码位置集合。\\
        h_C & \R^d & $[\mathrm{CLS}]$ 汇总表示。\\
    \bottomrule
    \end{longtable}
...
随机掩码集合为 $M$，目标
\begin{equation}
  \cL_{\rm MLM}=-\sum_{i\in M}\log p_\theta(x_i\mid x_{\setminus M}).
\end{equation}
它是条件伪似然，不是完整自回归联合似然；双向注意力允许每个被掩码位置使用左右上下文。

\section{下一句预测}
二分类标签 $y\in\{0,1\}$，$[\mathrm{CLS}]$ 表示为 $h_C$，
\begin{equation}
  p(y=1\mid A,B)=\sigma(w^Th_C+b),
\end{equation}
交叉熵梯度对 logit 为 $p-y$。总预训练目标是 $\cL_{\rm MLM}+\cL_{\rm NSP}$；共享编码器的梯度为两项之和。
