...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 1-2305.14314v1-Basis-QLoRA}
    \end{center}
    \vspace{0.8em}
...
c & \R & 低秩更新缩放系数。\\
Q(W_0) & \{0,\ldots,15\}^{m\times n} & NF4 量化码。\\
        s & \R_{>0} & 量化块尺度。\\
    \bottomrule
    \end{longtable}
...
对一块权重 $w\in\R^B$，令尺度
\begin{equation}
 s=\max_i|w_i|,\qquad u_i=w_i/s\in[-1,1].
\end{equation}
给定 $K=16$ 个码字 $q_1<\cdots<q_K$，量化索引
\begin{equation}
 k_i=\arg\min_k|u_i-q_k|,\qquad
 \widehat w_i=sq_{k_i}.
\end{equation}
若最大相邻间隔
\begin{equation}
 \Delta_{\max}=\max_k(q_{k+1}-q_k),
\end{equation}
且端点覆盖 $[-1,1]$，最近邻误差满足
\begin{equation}
 |w_i-\widehat w_i|\le\frac{s\Delta_{\max}}2,
 \qquad
 \|w-\widehat w\|_2^2\le
 \frac{Bs^2\Delta_{\max}^2}{4}.
\end{equation}
非均匀码本不是最小化最坏间隔，而是按权重分布把更多码字放在高概率区域，降低期望误差。

NF4 以标准正态分位区间构造近似等概率码字。若边界
\begin{equation}
 b_k=\Phi^{-1}(k/K),\qquad k=0,\ldots,K,
\end{equation}
区间代表值可取条件均值
\begin{equation}
 q_k=\E[Z\mid b_{k-1}<Z\le b_k],\qquad Z\sim\cN(0,1),
\end{equation}
...
第一层反量化为
\begin{equation}
 \widehat w_i=s_bq_{k_i}.
\end{equation}
若尺度本身被量化为 $\widehat s_b=s_b+e_b$，双重量化输出
\begin{equation}
 \widetilde w_i=\widehat s_bq_{k_i}.
\end{equation}
总误差分解
\begin{equation}
 w_i-\widetilde w_i
 =(w_i-s_bq_{k_i})-e_bq_{k_i}.
\end{equation}
因此
\begin{align}
 |w_i-\widetilde w_i|
 &\le |w_i-s_bq_{k_i}|+|e_b||q_{k_i}|\\
 &\le\frac{s_b\Delta_{\max}}2+|e_b|\max_k|q_k|.
\end{align}
若码字幅值不超过 $1$，尺度量化误差直接加到绝对误差上。双重量化节省每块尺度存储，
代价是所有块内元素共享同一个尺度误差，因而误差相关。

\section{量化基座上的 LoRA 梯度}

前向计算使用反量化基座
\begin{equation}
 h=(\operatorname{dequant}(Q(W_0))+cBA)x
 =(\widehat W_0+cBA)x.
\end{equation}
$W_0$ 冻结，故无需对不可微量化索引求梯度。令上游梯度 $g$，仍有
\begin{equation}
 \nabla_B\cL=c\,g(Ax)^T,\qquad
 \nabla_A\cL=c,B^Tg,x^T.
\end{equation}
但输入 $x$ 和上游 $g$ 是经过含 $\widehat W_0$ 的整网产生，故量化误差会间接改变 LoRA 的优化轨迹。

设 $E=\widehat W_0-W_0$，单层输出误差
\begin{equation}
 \|Ex\|_2\le\|E\|_2\|x\|_2\le\|E\|_F\|x\|_2.
\end{equation}
若后续网络 $F$ 是 $L_F$-Lipschitz，最终扰动
\begin{equation}
 \|F(\widehat W_0x)-F(W_0x)\|
 \le L_F\|E\|_2\|x\|.
\end{equation}

\section{存储量核算}

$P$ 个 4-bit 权重的主体存储为 $P/2$ 字节。若每 $B$ 个权重共享一个
$b_s$ bit 尺度，则额外
\begin{equation}
 \frac PB\frac{b_s}{8}\quad\text{字节},
\end{equation}
平均每权重 bit 数
\begin{equation}
 4+\frac{b_s}{B}.
...
\begin{equation}
 4+\frac{b_{ds}}{B}+\frac{b_c}{BB_s}\quad\text{bit/weight}.
\end{equation}
...
\end{equation}
分页把暂时不用的状态迁移到主存，不改变上述数学更新，只改变状态驻留位置。
若一次迁移 $M$ 字节、总线带宽 $B_w$，理想下界延迟为
\begin{equation}
 T_{\rm transfer}\ge M/B_w.
\end{equation}
