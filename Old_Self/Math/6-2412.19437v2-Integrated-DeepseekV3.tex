...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 6-2412.19437v2-Integrated-DeepseekV3}
    \end{center}
    \vspace{0.8em}
...
\section{无辅助损失的偏置平衡}
路由选择使用校正分数 $s_i+b_i$，但混合权重仍由原始 $s_i$ 计算。若专家 $i$ 负载超过目标，更新
\begin{equation}
  b_i\leftarrow b_i-\eta;qquad
  \text{负载不足时 }b_i\leftarrow b_i+\eta.
\end{equation}
偏置不经任务梯度且不进入专家输出数值，只改变 top-$K$ 集合。因此它类似离散约束的对偶变量更新，可减少显式平衡损失对主目标的干扰。

\section{多 token 预测}
除下一 token 外，同时预测未来 $D$ 个 token：
\begin{equation}
  \cL_{\rm MTP}=\frac1D\sum_{k=1}^{D}
  \left[-\log p_\theta(x_{t+k}\mid x_{\le t},\text{前级表示})\right].
\end{equation}
辅助头让当前表示包含更远未来信息。若权重过大，模型可能牺牲精确的一步条件分布；所以总目标采用主 NTP 损失加衰减的 MTP 权重。
