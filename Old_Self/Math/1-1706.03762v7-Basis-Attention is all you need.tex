...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 1-1706.03762v7-Basis-Attention is all you need}
    \end{center}
    \vspace{0.8em}
...
\end{align}
对固定频率 $\omega$，$[\sin((p+k)\omega),\cos((p+k)\omega)]$ 可由
$[\sin(p\omega),\cos(p\omega)]$ 乘一个只依赖 $k$ 的旋转矩阵得到，因而相对位移可被线性读取。

\section{残差与层归一化}
残差块 $y=x+F(x)$ 的 Jacobian 为 $I+J_F$，给深层梯度提供恒等通路。层归一化对单 token 的通道做
\begin{equation}
  \operatorname{LN}(x)=\gamma\odot\frac{x-\mu}{\sqrt{\sigma^2+\epsilon}}+\beta,
\end{equation}
它移除共同平移并控制尺度，但不同 token 之间不混合统计量。
