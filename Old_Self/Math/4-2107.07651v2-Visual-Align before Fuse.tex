...
\begin{document}
    \begin{center}
      {\LARGE\bfseries 4-2107.07651v2-Visual-Align before Fuse}
    \end{center}
    \vspace{0.8em}
    ...
    \midrule
    u_i,v_i & \R^d & 图像与文本全局嵌入。\\
        \tau & \R_{>0} & 对比温度。\\
        \theta,\bar\theta & -- & 学生与动量教师参数。\\
        m,\lambda & [0,1] & 动量系数与软标签混合系数。\\
    \bottomrule
    \end{longtable}
...
\end{equation}
学生目标分布混合硬标签 $y$ 与教师概率 $q$：
\begin{equation}
  \tilde y=(1-\lambda)y+\lambda q,
  \qquad
  \cL=-\sum_j\tilde y_j\log p_j.
\end{equation}
logit 梯度变为 $p-\tilde y$。当数据含多对合理匹配时，教师软分布可减轻假负样本，但若教师早期偏差大，也会把错误稳定地传给学生。

\section{图文匹配与掩码语言建模}
ALBEF 联合
\begin{equation}
  \cL=\cL_{\rm ITC}+\lambda_{\rm ITM}\cL_{\rm ITM}
  +\lambda_{\rm MLM}\cL_{\rm MLM}.
\end{equation}
ITC 对齐全局嵌入，ITM 在融合编码器上二分类，MLM 训练条件 token 分布。三项梯度共享视觉与文本主干；权重决定“可检索几何”和“细粒度跨模态推理”之间的优化折衷。
